...
public class ConnectionHandlerReactor<T> {
	...
	protected final TBGProtocol4Reactor _protocol;
	...
	private ConnectionHandlerReactor(SocketChannel sChannel, ReactorData<T> data, SelectionKey key) {
		_sChannel = sChannel;
		_data = data;
		_protocol = _data.getProtocolMaker().createR();
	...
	public static <T> ConnectionHandlerReactor<T> create(SocketChannel sChannel, ReactorData<T> data, SelectionKey key) {
		ConnectionHandlerReactor<T> h = new ConnectionHandlerReactor<T>(sChannel, data, key);
		h.initialize();
		return h;
	}
